...
export class ManageVendorComponent implements OnInit, AdminSettings {
	...
	ngOnInit() {
		if( ! this.globalService.vendors ) this.updateList();
	}
	...
	updateList() {
		...
		this.adminService.getVendors({orgId:this.globalService.getRequest().orgId}).subscribe((result:any[]) => this.globalService.vendors = result);
	}
